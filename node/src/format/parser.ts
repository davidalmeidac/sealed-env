...
import type { Mode, SealedFile, KdfParams } from '../core/types.js';
...
export function parseSealedFile(text: string): SealedFile {
  ...
  // ── Required fields
  const kdf = required(metadata, 'KDF');
  if (kdf !== 'argon2id') {
    throw new SealedEnvError('INVALID_FIELD', `sealed-env: unsupported KDF "${kdf}"`);
  }

  const kdfParams = parseKdfParams(required(metadata, 'KDF-PARAMS'));
...
function parseKdfParams(s: string): KdfParams {
  const m = s.match(/^t=(\d+),m=(\d+),p=(\d+)$/);
  if (!m) {
    throw new SealedEnvError(
      'INVALID_FIELD',
      `sealed-env: invalid KDF-PARAMS "${s}"`,
    );
  }
  const t = Number(m[1]);
  const mem = Number(m[2]);
  const p = Number(m[3]);
  if (!isFinite(t) || !isFinite(mem) || !isFinite(p) || t < 1 || mem < 1024 || p < 1) {
    throw new SealedEnvError('INVALID_FIELD', 'sealed-env: KDF parameters out of range');
  }
  return { t, m: mem, p };
}
